...
class ModerationConfig(object):
    ...
    inference_batch_size: int = 40 # change as needed
...
class ModerationModel(lit_model.BatchedModel):
    ...
    def _load_model(self, model_name):
        ...
        self.model = model_utils.load_pretrained(
            ...
            model_name,
            config=model_config,
            from_pt=True)  # here gets the config from the transformers
    ...
    def _scatter_embs(self, passed_input_embs, input_embs, batch_indices, offsets):
        ...
        scatter_indices = []
        for (batch_index, sentence_embs, offset) in zip(batch_indices, filtered_embs, offsets):
    ...
    def scatter_all_embeddings(self, inputs, input_embs):
        ...
        passed_input_embs = [ex.get("input_embs_" + self.config.prompt) for ex in inputs]
    ...
    def get_target_scores(self, inputs: Iterable[JsonDict], scores):
        ...
        for i, ex in enumerate(inputs):
            result = next((key for key, value in ex.items() if value == 1), arg_max[i])
            label = result
    ...
    def predict_minibatch(self, inputs: Iterable[JsonDict]):
        ...
        with (tf.GradientTape(watch_accessed_variables=self.config.compute_grads) as tape):
            ...
            batched_outputs["probas"] = tf.nn.softmax(out.logits, axis=-1)  # squeeze is not needed because of nn
