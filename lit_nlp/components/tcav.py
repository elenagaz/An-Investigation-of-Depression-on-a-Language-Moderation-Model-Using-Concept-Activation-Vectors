...
from lit_nlp.my_model_moderation.utils import compare_predictions
import numpy as np
import scipy.stats
  ...
  **Note: TCAV calls the model multiple times. It is _strongly_ recommended that
  ...
  """
    ...
    def get_predictions(
        ...
        model_name = model.wrapped.model.name_or_path
        model_name = model_name.replace('/', '_')
        relative_path = os.path.join('..', 'my_model_moderation', f'{model_name}_prediction_cache.pkl')
        ...
        if model_name == 'KoalaAI_Text-Moderation':
            if os.path.isfile(file_path) and os.path.getsize(file_path) > 0:
                ...
                for ex, preds in zip(inputs, predictions, strict=True):
                    overrides = {}  # saves the preds for each example
                ...
                predictions1 = list(model.predict(modified_inputs))
                compare_predictions.compare_structures(predictions, predictions1)
    ...
    def _run_default_tcav(
        ...
        concept_results = []  # saves the length of concept (includes score, cos_sim, dot_prods, accuracy, cav)
        ...
        logging.info('Result:')
        res = results['result']
        logging.info(res['score'])
        logging.info('Random Mean:')
        logging.info(results['random_mean'])
    ...
    def _run_tcav(self,
        ...
        # cosine can be -1 = identical, 0 = orthogonal/unrelated,  1 = opposite
    ...
    def get_trained_cav(self, x, y, test_size, random_state=None):
        ...
        for pred_val, test_val in zip(y_pred, y_test):
            if pred_val == test_val:
                correct_count += 1
        accuracy = correct_count / len(y_test)  # cav accuracy
    ...
    def get_dir_derivs(self, cav, dataset_outputs, grad_layer, grad_class_key,
        ...
        dir_derivs = []
        # dataset outputs are all the examples loaded to LIT
        for o in dataset_outputs:
            if o[grad_class_key] == class_to_explain:  # check all the inputs for the class to explain
                grad = o[grad_layer]
    ...
    def compute_local_scores(self, cav, dataset_outputs, emb_layer):
        ...
        dot_prods = [np.dot(flattened_cav, o[emb_layer]) for o in
                     dataset_outputs]
